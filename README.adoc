...
These dotfiles are intended for use in Bash on Debian-based systems, but I've made some effort to make them as cross-platform compatible as possible. Most of the shell scripts are POSIX-compliant and will therefore run in all Unix shells. They can be used on Windows too, via a Linux emulator like https://gitforwindows.org/[Git Bash for] or a virtual machine like WSL.

TIP: On Debian systems, my link:https://github.com/kieranpotts/bootstrap[bootstrap scripts] can be used to install all dependencies, so automating the following steps. For Git Bash on Windows, my link:https://github.com/kieranpotts/devtools[devtools] repository bundles most of the required programs, but some of the below steps still need to be done manually.
...
It is RECOMMENDED to install Git Large File Storage (LFS). This is a Git extension that removes large files from Git repositories and stores them elsewhere, cross-referenced from the Git repositories using pointers. Git LFS is included in the distribution of link:https://gitforwindows.org/[Git for Windows], while link:https://github.com/git-lfs/git-lfs/blob/main/INSTALLING.md[packages are available for popular Linux distributions]. Once installed, run the following command to set up Git LFS globally in your local Git instance:
...
Configuration files are included for various command line development tools. To use these configurations, you will need to create symlinks to them, from the filesystem locations the programs expect them to be.

.WSL and Unix-like systems
[source,sh]
----
# LazyGit
ln --symbolic --force /path/to/dotfiles/etc/lazygit/config.yml ~/.config/lazygit/config.yml

# Nvim
mkdir -p ~/.config/nvim
ln --symbolic --force /path/to/dotfiles/etc/nvim/init.vim ~/.config/nvim/init.vim
----
...
The first solution depends on link:https://ohmyposh.dev/[Oh-My-Posh], a cross-platform prompt theming framework. It means the prompt theme can be used in Powershell, Git Bash, WSL, and other shells. The theme is enabled via the following command in the `~/local.bashrc` file. You just need to uncomment the line to enable it.
...
Alternatively, you can enable the `git-prompt.sh` file, which adds Git repo information to the standard prompt line. Uncomment the following lines in the `~/local.bashrc` file, instead.
