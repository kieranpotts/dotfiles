...
* And an extensive suite of useful Git aliases – things like `git sync`, `git amend`, `git squash`, `git fixup`, and `git undo`.

I install these dotfiles on Debian-based systems, but I've made some effort to make them as cross-platform compatible as possible. Most of the shell scripts are POSIX-compliant and will therefore run in all UNIX shells. They can be used on Windows too, via a Linux emulator like Git Bash or a virtual machine like WSL.

These dotfiles complement my link:https://github.com/kieranpotts/devtools[devtools] – additional tools and configuration for use directly on Windows.
...
TIP: On Debian systems, my link:https://github.com/kieranpotts/bootstrap[bootstrap scripts] can be used to install all REQUIRED and OPTIONAL programs, so automating the following steps. For Git Bash on Windows, my link:https://github.com/kieranpotts/devtools[devtools] bundle most of these programs, but some of the below steps still need to be done manually.
...
It is RECOMMENDED to install Git Large File Storage (LFS). This is a Git extension that removes large files from repositories and stores them elsewhere, cross-referenced from the Git repositories using pointers. Git LFS is included in the distribution of link:https://gitforwindows.org/[Git for Windows], while link:https://github.com/git-lfs/git-lfs/blob/main/INSTALLING.md[packages are available for popular Linux distributions]. Once installed, run the following command to set up Git LFS globally in your local Git instance:
...
Then commit and push as normal. To convert existing files previously committed to the repository use the `git lfs migrate` command. See the https://github.com/git-lfs/git-lfs/blob/main/docs/man/git-lfs-migrate.adoc[Git LFS documentation] for further instructions, and more link:https://docs.github.com/en/repositories/working-with-files/managing-large-files/about-large-files-on-github[about large files on GitHub].
...
My dotfiles include configurations for LazyGit and NeoVim, while Delta and Oh-My-Posh may be used to enhance Git and the terminal prompt, respectively. For Debian systems, you can use my link:https://github.com/kieranpotts/bootstrap[bootstrap scripts] to install these programs, or follow the instructions from the links above to install manually. For Git Bash on Windows, most of these programs are bundled with my link:https://github.com/kieranpotts/devtools[devtools] – except for NeoVim, which is not available as a standalone binary.
...
The install script will install and enable the dotfiles. The first time you do this, you will need to exit your terminal program then restart it. Doing so will create a new login shell, which will load the newly-installed dotfiles at startup.
...
This repository's `bin` directory contains various `git-*` shell scripts that are mapped to Git aliases. It is RECOMMENDED you add the `bin` directory to your system PATH environment variable, to make these Git commands available from the command line. For Bash, this is done automatically via configuration in the installed `.bashrc` file. To make the Git aliases available in the Git Bash emulator on Window, you will need to manually update your PATH environment variable.
...
To use the LazyGit and NeoVim configurations in Git Bash for Windows, run Windows Powershell in administrator mode and execute the following commands, changing the filesystem paths as required.
...
Alternatively, you can enable the `git-prompt.sh` file, which adds Git repo information to the standard prompt line. Uncomment the following line in the `~/local.bashrc` file, instead.
...
|Change to `0` to apply `--no-verify` flag to aliased `git commit` operations done by any of the Git aliases.
|Bash
|===
...
|`reload!`
|`. ~/.bashrc`
|Reload the Bash shell run command script, whenever you make changes to its config.
|Bash
...
*Acknowledgments*: Inspiration has been taken – and code shamelessly copied – from some of these related projects:
