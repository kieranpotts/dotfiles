...
The `bin/update!` script will establish various symlinks in your home directory for the Unix dotfiles. For example, `~/.gitconfig` will be symlinked to `~/dotfiles/dist/global.gitconfig`. You SHOULD NOT edit the dotfiles symlinks and you SHOULD NOT edit the contents of the `~/dotfiles/dist` directory, either, unless you intend to commit the changes to the dotfiles repository and share the configuration changes across all development environments.
