...
Bash reads a file called `~/.bashrc` ("Bash run command"), if it exists, when the shell is run in _interactive non-login_ mode, for example when `bash` is called to execute a script.
